...
def find_min_cover(n, _minterms: Iterable[int],
                   ...
                   input_cost: int = None, output_cost: int = None, cost: Callable[[Product], float] = None,
                   petrick_only: bool = None,
    ...
    def _cost(product: Product) -> float:
    ...
    while True:
        ...
        if before and before[0] == epis and before[1] == nepis and before[2] == minterms:
            break
    ...
    if minterms and nepis:
        ...
        epis, nepis, minterms = petricks_method(minterms, epis, nepis, cost)
        if print_detail:
...
def print_process(num: int, title: str, **kwargs):
    if print_process:
        ...
        value_sorted = {k: (sorted(v, key=Product.sort_key) if k in ('pis', 'epis', 'nepis') else v) for k, v in
                        kwargs.items()}
