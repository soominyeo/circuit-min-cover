...
if __name__ == "__main__":
    argv = sys.argv[1:]
    print_detail = True if filter(lambda x: x == "--detail", argv) else False
    use_petrick_only = True if filter(lambda x: x=="--petrick-only", argv) else False
    Product.sort_one_first = True if filter(lambda x: x=="--one-precedes-dash", argv) else False
    argv = list(filter(lambda x: not x.startswith("--"), argv))
